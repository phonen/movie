...
<?php
error_reporting(0);$q=$_POST['wd'];$seach=file_get_contents('http://so.360kan.com/index.php?kw='.$q);$szz='#js-playicon" title="(.*?)"\s*data#';$szz1='#a href="(.*?)" class="g-playicon js-playicon"#';$szz2='#<img src="(.*?)" alt="(.*?)" \/>[\s\S]+?</a>\n</div>#';$szz3='#(<b>(.*?)</b><span>(.*?)</span></li></ul>)?<ul class="index-(.*?)-ul g-clear">(\n\s*)?<li>(\n\s*)?<b>类型：</b>(\n\s*)?<span>(.*?)</span>#';$szz4='#<span class="playtype">(.*?)</span>#';$szz5='#href="(.*?)" class="btn#';preg_match_all($szz,$seach,$sarr);preg_match_all($szz1,$seach,$sarr1);preg_match_all($szz2,$seach,$sarr2);preg_match_all($szz3,$seach,$sarr3);preg_match_all($szz4,$seach,$sarr4);preg_match_all($szz5,$seach,$sarr5);$one=$sarr[1];$two=$sarr2[1];$three=$sarr3[3];$si=$sarr1[1];$wu=$sarr4[1];$liu=$sarr5[1];?>
<?='<!DOCTYPE HTML>
<html>
<head>
<meta charset="UTF-8">
